import { AsyncAction, Dispatch, StateFetcher } from "types/app-types";
...
export const updateAuthData = actionCreator<AuthData | null>(
  "AUTH_UPDATE_DATA"
);
...
export const basicSignup =
  (body: SignupRequestBody): AsyncAction =>
  async (dispatch: Dispatch) =>
    authRequest(body, "signup/basic", dispatch);

export const basicLogin =
  (body: LoginRequestBody): AsyncAction =>
  async (dispatch: Dispatch) =>
...
const authRequest = async (
  ...
  try {
    ...
    const response = await publicRequest<
      SignupRequestBody | LoginRequestBody,
      AuthData
    >({
      url: endpoint,
      method: "POST",
...
export const logout =
  (): AsyncAction => async (dispatch: Dispatch, getState: StateFetcher) => {
    try {
