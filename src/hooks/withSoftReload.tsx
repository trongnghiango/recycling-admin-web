...
type SoftReloadablePageInitialProps<TProps> = OmitSoftReloadProps<TProps> & {
  context: NextPageContext;
};
export type NextPageWithInitialProps<P, IP = P> = NextPage<P, IP> &
  Required<Pick<NextPage<P, IP>, "getInitialProps">>;
...
const withSoftReload = <TProps extends SoftReloadProps>(
  ...
  async function getInitialProps(
    ctx: NextPageContext
  ): Promise<SoftReloadablePageInitialProps<TProps>> {
    return {
  ...
  const omitContextFromProps = ({
    context,
    ...props
  }: SoftReloadablePageInitialProps<TProps>): OmitSoftReloadProps<TProps> =>
    props as any;
  const NewPage: NextPageWithInitialProps<
    SoftReloadablePageInitialProps<TProps>
  > = (props) => {
    ...
    return (
      <Page
        {...({ ...initialProps, softReload } as Omit<
          TProps,
          keyof SoftReloadProps
        > &
          SoftReloadProps as TProps)}
      />
    );
  };
