...
import { len, validateEmail } from "utils/utils";

import axios from "axios";
import { signIn } from "domains/auth/auth.services";
import { getUserFromCookies, setUserToCookies } from "utils/JWT";
import { useAuth } from "@/auth-context/AuthContext";
import { deleteCookie } from "cookies-next";
import Loading from "@/components/shared/loading";

export default function SignIn() {
  const [spin, set] = useState<boolean>(false);
  ...
  const handleCredentialChange =
    (name: string) => (e: ChangeEvent<HTMLInputElement>) => {
      ...
      console.log("val:: inp::", name, e.target.value);
      setCredentials({
        ...credentials,
        isEmailError: false,
        emailError: "",
  ...
  const handleBasicLogin = async () => {
    ...
    console.log("submit Info::", validations);
    if (!!error) {
      alert("LỖI ĐĂNG NHẬP");
  ...
  return (
    <>
      <section className="relative min-h-screen flex items-stretch text-white ">
        <div
          className="tablet:flex w-1/2 hidden bg-gray-500 bg-no-repeat bg-cover relative items-center"
          style={{
            backgroundImage:
        ...
        <div
          className="tablet:w-1/2 w-full flex items-center justify-center text-center mobile:px-16 px-0 z-0"
          style={{ backgroundColor: "#161616" }}
        >
          <div
            className="absolute tablet:hidden z-10 inset-0 bg-gray-500 bg-no-repeat bg-cover items-center"
            style={{
              backgroundImage:
                "url(https://images.unsplash.com/photo-1577495508048-b635879837f1?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=675&q=80)",
            }}
          ...
          <div className="w-full py-6 z-20">
            <h1 className="my-6 text-4xl text-white font-bold">Kool Smile</h1>
            ...
            <form
              ...
              className="sm:w-2/3 w-full px-4 tablet:px-0 mx-auto"
              noValidate
              autoComplete="off"
            >
              {/* email wrapper */}
            ...
            <div className="p-4 text-center right-0 left-0 flex justify-center space-x-4 mt-16 tablet:hidden ">
              <a href="#">
                <svg
                  fill="#fff"
                  xmlns="http://www.w3.org/2000/svg"
